...
func _ready():
	...
	# SN - watch this double indent, doesn't matter here but could be problematic if you are not aware
...
func _on_reset_button_pressed():
	var reset = load("res://Scenes/9level3.tscn").instance()
	add_child(reset)
	remove_child(self)
	
# SN - I don't think this button is linked properly, check also for higher levels, might be why you are quitting out
...
#		$d4/correct.visible = true
	...
	if ($a1.cell_value) == int(0) and ($a2.cell_value) == int(0) and ($a3.cell_value) == int(1) and ($a4.cell_value) == int(0) and ($b1.cell_value) == int(0) and ($b2.cell_value) == int(0) and ($b3.cell_value) == int(0) and ($b4.cell_value) == int(1) and ($c1.cell_value) == int(0) and ($c2.cell_value) == int(1) and ($c3.cell_value) == int(0) and ($c4.cell_value) == int(0) and ($d1.cell_value) == int(1) and ($d2.cell_value) == int(0) and ($d3.cell_value) == int(0) and ($d4.cell_value) == int(0):
		var complete = load("res://Scenes/9level3_complete.tscn").instance()
		add_child(complete)
		remove_child(self)
